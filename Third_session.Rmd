...
The main plotting method consists of elements that we can add. The first element is the *tm_shape()* function specifying the spatial object, and then we can add a series of elements speficying layers in the visualisation. They can include polygons, symbols, polylines, raster, and text labels as base layers. Last week we saw how we could use *tm_polygon()* to add polygons. Let's add such a layer:
...
There are different ways of creating small multiples with *tmap* as you could see in the vignettes for the package, some of which are quicker but a bit more restricted. Here we are going to use *tmap_arrange()*. With *tmap_arrange()* first we need to create the maps we want and then we arrange them togeter.
...
Notice that we are not plotting the maps, we are storing them into R objects. This way they are saved, and you can call them later, which is what we need in order to plot them together using the `tmap_arrange()` function. 
...
*Homework 2: Discuss which of these classification methods gives you the best visualisation of crime in Manchester city*
...
#Always remember to load the required library
library(ggmap)
#Then we will set the location we want to focus on, around the University of Manchester (uoM)
UoM <- c(lon= -2.233885, lat= 53.466852)
#Then we download the basemap centred on those coordinate, at the appropriate zoom level, and using the stamen toner background (if some of this code is unclear review last week materials)
mapUoM <- get_map(UoM, zoom = 15, scale= 1, source = "stamen", maptype = "toner")
#Then we use ggmap to plot the crimes as dots over the basemap we just downloaded. Notice we are still mapping the colour of the dots to the type of crime
...
#The final facet_wrap() function specifies that we want to face the map, to create the small multiples
```
...
Since we didn't modify the *tmap_mode* from our last call we are still running on the view rather than the plot format. We can go back to the plot format with the following call.
...
Again we use the `tm_shape()` and `tm_dots()` functions to specify the presentation of the map, with `tm_shape()` specitying the shape object (in this case crime_sf), and `tm_dots()` to draw symbols, including specifying the color, size, and shape of the symbols. In this case we adjust the tranparency with the `alpha` parameter. 
...
![](https://media.giphy.com/media/E67TuM1Js9VBe/giphy.gif)
